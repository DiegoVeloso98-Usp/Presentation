...
\begin{frame}{}
    ...
    \begin{itemize}[label=\textbullet]
        ...
        \item \textbf{Python}: A programming language used for scripting and automation of the workflow.
...
\begin{frame}{}
    ...
    \end{figure}
    PETSc (Portable, Extensible Toolkit for Scientific Computation) is designed for the \textbf{scalable} solution of scientific applications modeled by partial differential equations (\textbf{PDEs}). 
    It is a powerful and convenient interface for:
    \begin{itemize}[label=\textbullet]
...
\begin{frame}{}
    \frametitle{GMSH,HDF5, Python, Paraview}
...
\begin{frame}{}
  ...
  \begin{adjustbox}{max width=\textwidth, max totalheight=\textheight, keepaspectratio}
    \begin{minipage}{\textwidth}
      \scriptsize
  \begin{algorithmic}[1]
    \Ensure Newton-raphson solution $\Delta\bm{u}^{k+1}$
      \State Compute $R_0 = \bm{R}(\bm{u}^k)\cdot\Delta\bm{u}^{k+1}$
      \State $\eta_0 \gets 1,\ \rho \gets 0.5$
      \For{$n = 0$ {\bfseries to} $N_{\max}^{LS}$}
        \State $\bm{u}_{\text{n}} \gets \bm{u}^k + \eta_n\,\Delta\bm{u}^{k+1}$
        \State Assemble residual $\bm{R}(\eta_n) = \bm{F}(\eta_n)_{int} - \bm{F}_{ext}$  
        \State Compute $R(\eta_n) = \bm{R}(\eta_n)\cdot\Delta\bm{u}^{k+1}$
        \If{$|R(\eta_n)| \le \rho\,|R_0|$}
          \State \textbf{break}
        \Else
          \State $\alpha \gets R_0 / R(\eta_n)$
          \If{$\alpha < 0$}
            \State $\eta_n \gets \alpha/2 + \sqrt{(\alpha/2)^2 - \alpha}$
          \Else
            \State $\eta_n \gets \alpha/2$
          \EndIf
        \EndIf
        \State $n \gets n + 1$
      \EndFor
      \State $\bm{u}^{\,k+1} \gets \bm{u}^k + \eta_n\,\Delta\bm{u}^{k+1}$
  \end{algorithmic}
    \end{minipage}
  \end{adjustbox}
...
\begin{frame}{}
    ...
    Taylor expansion around \(\hat{\bm{d}}_n\)
    \begin{equation}
    \varPi_{n+1}(\hat{\bm{u}}^i,\hat{\bm{d}})
    = \frac12\,\Delta\hat{\bm{d}}^{\mathsf T}\,Q^i\,\Delta\hat{\bm{d}}
    + \Delta\hat{\bm{d}}^{\mathsf T}\,q^i
...
\begin{frame}{}
    ...
    the constant element dissipation matrix \(\Phi_e\) is given by
\begin{equation}
  \Phi_e := 
  \begin{cases}
